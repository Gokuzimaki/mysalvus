...
if($dhash=="true"||$displaytype=="forceuserreset"){
	...
	// echo $displaytype;
	...
	}else if($displaytype=="getserviceproviders"){
		if($test!==""){
		...
		// echo $limit."<br>";
		$type[0]="activationstatus";
		$type[1]="active";
		$outs=getAllUsers('viewer',"all",'serviceprovider','',$type);
		...
		if($numrows>0){
			...
			$msg="Retrieval Successful";
		 	echo json_encode(array("success"=>"true","msg"=>"$msg","catdata"=>$outs['catdata'],"totalresults"=>"$numrows"));
		 	if($test!==""){
				$scriptout='
					<script>
						var sdata='.json_encode(array("success"=>"true","msg"=>"$msg","catdata"=>$outs['catdata'],"totalresults"=>"$numrows")).';
					</script>
				';
				echo $scriptout;
			}	
			...
			$msg="Retrieval fail";
			echo json_encode(array("success"=>"false","msg"=>"$msg","catdata"=>$outs['catdata'],"totalresults"=>"0"));
		...
		if($email!==""){
		    ...
		    if($cmail['testresult']=="matched"){
		        ...
		        if($udata['status']=="active"){
			        ...
			        ';
			        $emailout=generateMailMarkUp("mysalvus.com","$email","$title","$content","$footer","");
			        // echo $emailout['rowmarkup'];
			        $toemail=$email;
			        $headers = "MIME-Version: 1.0" . "\r\n";
			        $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
			        $headers .= 'From: <no-reply@mysalvus.com>' . "\r\n";
