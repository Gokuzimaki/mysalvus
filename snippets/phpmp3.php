...
class PHPMP3
{
	...
	public function extract($start, $length)
	{
		...
		while($currentStrPos < $maxStrLen)
		{
			...
			{
				$parts[] = $this->binaryTable[$str[$i]];
				// echo $this->binaryTable[$str[$i]]." $i <br>";
			}
			if($parts[0] == '11111111')
			{
				$a = $this->doFrameStuff($parts);
	...
	private function doFrameStuff($parts)
	{
		...
		$seconds = 0;
		$errors = array();
		// echo substr($parts[1], 3, 2)."part1 3,2<br>";
		...
		//Get Layer
		$layer=0;
		// echo substr($parts[1], 5, 2)."part1 5,2<br>";
		...
		$bitArrayNumber=0; //  TODO: Remove this line or set a value
		if($audio == 1)
		{
			switch($layer)
			{
